...
fn main() {
    ...
    let mut circle : VecDeque<i32> = VecDeque::with_capacity(last_marble_value as usize);
    ...
    for current_marble_value in 1..=last_marble_value {
        let circle_len = circle.len() as i32;
        if current_marble_value % 23 != 0 {
            ...
            let bonus_marble_idx : i32 = (current_marble_idx - 7 + circle_len) % circle_len;
